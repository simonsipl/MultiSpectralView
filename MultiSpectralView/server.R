...
lamp_m <- reactive({
    ...
    else
    {
        ...
        path_lamp = paste('./Lamps/',lamp_name , sep = "")
        lamp <- read.csv(path_lamp)
        
        lamp_mean = mean(lamp$Emission)/100
  ...
  output$filterTable <- renderTable({
    ...
    else
    {
      ...
      if(k_ex != "Brak")
      {
        resultTable = c(p_ex,k_ex,k_ex_l,resultTable)
      }
      else
      {
      ...
      if(k_em != "Brak")
      {
        resultTable = c(k_em,p_em,k_em_l,resultTable)
      }
      else
      {
        resultTable = c('','','',resultTable)
      ...
      rownames(t)=c("Quality index","Filter [%]","Lamp with Filter [%]")
